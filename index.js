...
function addGamesToPage(games) {
  ...
  for (const game of games) {
    ...
    card.innerHTML = `
        <img class="game-img" src="${game.img}" width="100">
        <h3>${game.name}</h3>
...
contributionsCard.innerHTML = `
    <h3>${contributions.toLocaleString("en-US")}</h3>   
`;

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
const pledgedAmt = GAMES_JSON.reduce((acc, game) => {
  return acc + game.pledged;
}, 0);
// set inner HTML using template literal
raisedCard.innerHTML = `
<h3>$${pledgedAmt.toLocaleString("en-US")}</h3>   
`;
...
let unfundedStr = `A total of $800,268 has been raised for 11 games. Currently, ${
...
const strDescription = document.createElement("div");
strDescription.innerHTML = `
        <p>${unfundedStr}</p>
    `;
descriptionContainer.appendChild(strDescription);
...
//customization: navigating to our game section on nav bar
const topAllBtn = document.getElementById("top-all-btn");
topAllBtn.addEventListener("click", () => {
  showAllGames();
